...
import { updateCategoryMaxPerTeamAction } from "@/lib/actions/auction.actions";
import { inputClass, buttonPrimary, buttonSecondary } from "@/lib/ui";

export function EditCategoryMaxPerTeamForm({
  auctionId,
  categoryId,
  maxPerTeam,
}: {
  auctionId: string;
  categoryId: string;
  maxPerTeam: string | null;
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [value, setValue] = useState(maxPerTeam ?? "");
  ...
  if (!open) {
    return (
      ...
      >
        {maxPerTeam ? `${maxPerTeam}/team (edit)` : "Set cap"}
      </button>
    );
  }
  ...
  async function handleSave() {
    setLoading(true);
    setError(null);
    const result = await updateCategoryMaxPerTeamAction(
      auctionId,
      categoryId,
