...
/** Ported from the web app's hooks/useBidTiming.ts (verbatim logic, no DOM
 * dependency to begin with) — shared timing/amount math for the bid control.
 * The countdown ticks locally (re-rendering every 500ms) while the actual
 * cooldown deadline and current bid always come from the server-driven
 * player state. */
